...
Each day you write a sasso about the day, with photos if you took any, and pull out of it the handful of things that actually mattered. Those pieces accumulate into one continuous record you can read end to end, filter by subject, and zoom out from a day to a week to a month to a year.
...
**Keep permanent intentions somewhere durable.** Bucket lists, things you want to buy, things you want to change about how you work — these currently live in scattered notes and messages to yourself. They deserve a shelf.
...
|---|---|
| **Stele** | the app |
| **Nota** | the one content type. Markdown, photos, optionally a date. Lives in a tavola |
...
| **Sasso** | a dated note in the diario — prose, photos and its highlights |
...
**3. The archive outlives the app.** The value of this data increases every year — year twelve is worth far more than year one — which means a phone dying, the framework breaking, or you losing interest in maintaining the code must never cost you the archive. Everything must be exportable into a form that rebuilds completely, images included, and that export is a first-class feature rather than a button nobody has pressed.
...
**5. One note, many presentations.** There is a single content type. A note holds Markdown, photos, and optionally a date. A tavola is a folder of notes.

The **diario** is the one tavola whose notes are *required* to have a date, and it is what the main page renders. That is the only thing that makes it special. Notes in any other tavola may carry a date too — it simply isn't demanded of them. No tavola has a capability another lacks; everything is built against the note, so the editor, Markdown, photos, search and export work identically everywhere. Since the main page already surfaces the diario, it can be hidden from the scaffale rather than listed twice.
...
**The day list.** Below everything, an infinite list of your diario notes in reverse chronological order, **separated by week**. Each entry is a card: the first photo as a small thumbnail if there is one, a title, and the first ~200 characters as a preview.
...
**Photos.** A couple per sasso. Compressed on import and copied into the app's own storage, so that whatever the gallery later does with the original — moves it, re-encodes it, offloads it — the sasso keeps its picture. Collages are made outside Stele in Ente Photos and come in as a single finished image.
...
It also carries the activity grid for the full year, and the reflections written at week and month level.
...
A tavola is a folder of notes, customised with a colour and an emoji: bucket list, things to buy, things to improve, productivity plan. The notes inside are ordinary notes — the same editor, the same Markdown, the same photos as a sasso — and you edit them whenever you want.
...
Stele exports all of your data: every note, scaglia, tag, tavola and image, in one JSON file, complete enough to rebuild the app's contents from nothing.
